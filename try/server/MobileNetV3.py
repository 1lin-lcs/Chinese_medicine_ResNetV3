...
class Bottleneck(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size,exp_channels,stride,se=True,nl="HS"):  #se?
        ...
        self.stride=stride
        if se:
            self.se=SEModule(exp_channels).to(device)
        else:
            self.se=None
        self.conv1=nn.Conv2d(in_channels,exp_channels,kernel_size=1,stride=1,padding=0,bias=False)
        #BatchNorm2d是Normalization层，这个exp_channels是input_size
        ...
        #先初始化一个空序列，然后改造其为残差连接
        self.shortcut=nn.Sequential()
        #只有步长为1且输入输出通道不相同时才采用跳跃来连接（想一下跳跃连接的过程，输入输出通道相同这个跳跃连接就没有意义了）
...
class MobileNetV3_large(nn.Module):
    ...
    def forward(self,x):
        ...
        out=self.conv4(out)
        #因为原论文中最后是卷积层来实现全连接的效果，维度是四维的，后两维是1，在计算损失函数的时候要求是二维的，因此在这里要做一个resize
...
class MobileNetV3_small(nn.Module):
    ...
    def forward(self,x):
        ...
        out=Hswish(self.conv3(out))
        out=self.conv4(out)
        #因为原论文最后一层是卷积层来实现全连接的效果，维度是四维的，后两维是1，在计算损失函数的时候要求二维，因此在这里需要做一个resize
        a,b=out.size(0),out.size(1)
        out=out.view(a,b)
        return out
